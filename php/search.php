...
END;
    ...
    $rezultat=@$polaczenie->query("SELECT NAZWA_LINII, ID_LOTU, SKAD, DATA_ODLOTU, GODZINA_ODLOTU, DOKAD, DATA_PRZYLOTU, CZAS_PRZYLOTU, WOLNE_MIEJSCA, Uwagi FROM LOTY, LINIE_LOTNICZE, SAMOLOTY WHERE SAMOLOTY.ID_LINII=LINIE_LOTNICZE.ID_LINII AND LOTY.ID_SAMOLOTU=SAMOLOTY.ID_SAMOLOTU AND (LOTY.SKAD LIKE '$M_startu' AND LOTY.DOKAD LIKE '$M_ladowania' AND (LOTY.DATA_ODLOTU='$D_startu' OR LOTY.DATA_PRZYLOTU='$D_ladowania'))");
    ...
    for ($i = 1; $i <= $ile; $i++)
    {
        ...
        $uwagi= $row['Uwagi'];
        $link = "<a href=\"flight_managment.php?id=$id\">Zarządzaj</a>";
